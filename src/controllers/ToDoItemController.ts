...
export class ToDoItemController implements ToDoItemControllerI {
	...
	public createToDoItem(req: Request, res: Response): void {
		...
		ToDoItemController.toDoItemDao
		...
		if (req.query.attrib) {
			if (req.query.attrib === 'deadline') {
				if (start && end && !before && !after) {
					console.info(
						`todo: findAllToDoItems() ${req.query.attrib} ${req.query.start} ${req.query.end}`
					);
					ToDoItemController.toDoItemDao
						.findToDoItemsBetweenDealines(Number(start), Number(end), req.query.sort)
						.then((items) => res.json(items))
						.catch((status) => res.json(status));
				} else if (before && !start && !end && !after) {
					console.info(`todo: findAllToDoItems() ${req.query.attrib} ${req.query.before}`);
					ToDoItemController.toDoItemDao
						.findToDoItemsBeforeDeadline(Number(before), req.query.sort)
						.then((items) => res.json(items))
						.catch((status) => res.json(status));
				} else if (after && !start && !end && !before) {
					console.info(`todo: findAllToDoItems() ${req.query.attrib} ${req.query.after}`);
					ToDoItemController.toDoItemDao
						.findToDoItemsAfterDeadline(Number(after), req.query.sort)
						.then((items) => res.json(items))
						.catch((status) => res.json(status));
				} else if (req.query.sort) {
					console.info(`todo: findAllToDoItems() ${req.query.attrib} ${req.query.sort}`);
					ToDoItemController.toDoItemDao
						.findAllToDoItems(req.query.attrib, req.query.sort)
						.then((items) => res.json(items))
						.catch((status) => res.json(status));
				} else {
				...
				if (start && end && !before && !after) {
					...
					ToDoItemController.toDoItemDao
						.findToDoItemsBetweenDates(Number(start), Number(end), req.query.sort)
						.then((items) => res.json(items))
						.catch((status) => res.json(status));
				} else if (before && !start && !end && !after) {
					console.info(`todo: findAllToDoItems() ${req.query.attrib} ${req.query.before}`);
					ToDoItemController.toDoItemDao
						.findToDoItemsBeforeDate(Number(before), req.query.sort)
						.then((items) => res.json(items))
						.catch((status) => res.json(status));
				} else if (after && !start && !end && !before) {
					console.info(`todo: findAllToDoItems() ${req.query.attrib} ${req.query.after}`);
					ToDoItemController.toDoItemDao
						.findToDoItemsAfterDate(Number(after), req.query.sort)
						.then((items) => res.json(items))
						.catch((status) => res.json(status));
				} else if (req.query.sort) {
					console.info(`todo: findAllToDoItems() ${req.query.attrib} ${req.query.sort}`);
			...
			ToDoItemController.toDoItemDao
				.findAllToDoItems('createdAt', req.query.sort)
				.then((items) => res.json(items))
				.catch((status) => res.json(status));
		}
	}
	...
	public findToDoItemById(req: Request, res: Response): void {
		...
		ToDoItemController.toDoItemDao
			.findToDoItemById(new ObjectId(req.params.id))
			.then((item) => res.json(item))
			.catch((status) => res.json(status));
	}
	...
	public updateToDoItemById(req: Request, res: Response): void {
		...
		ToDoItemController.toDoItemDao
			.updateToDoItemById(new ObjectId(req.params.id), req.body)
	...
	public deleteToDoItemById(req: Request, res: Response): void {
		...
		ToDoItemController.toDoItemDao
			.deleteToDoItemById(new ObjectId(req.params.id))
			.then((status) => res.json(status))
			.catch((status) => res.json(status));
	}
}
